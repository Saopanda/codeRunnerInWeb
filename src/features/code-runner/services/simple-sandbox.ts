...
export class SimpleSandboxManager {
  ...
  public async executeCode(
    ...
    language: 'javascript' | 'typescript' | 'php' | 'python' = 'javascript'
  ): Promise<void> {
    if (this.executionId) {
    ...
    // 开始性能监控
    const stopExecutionMonitoring = performanceMonitor.startExecutionMonitoring(
      code,
      language
    )
    ...
    if (language === 'php') {
      try {
        ...
        store.setExecutionState({
          ...
          firstExecutionTime:
            store.executionState.firstExecutionTime || executionTime,
        })
    ...
    if (language === 'typescript') {
      try {
        ...
        // 开始编译性能监控
        const stopCompilationMonitoring =
          performanceMonitor.startCompilationMonitoring(code, 'typescript')
    ...
    try {
      ...
      // 使用安全管理器进行代码分析
      const securityCheck =
        securityManager.createSecureExecutionEnvironment(code)
  ...
  private createSecureGlobal(
    baseGlobal: Record<string, unknown>
    ...
    for (const apiName of protectedAPIs) {
      if (secureGlobal[apiName]) {
        ...
        secureGlobal[apiName] = new Proxy(
          originalAPI as Record<string, unknown>,
          {
            get(target, prop) {
              // 检查API访问权限
